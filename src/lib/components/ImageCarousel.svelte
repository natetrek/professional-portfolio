...
{#if !isMobile}
  <!-- Non-mobile display -->
  <div id="carousel-container" class="carousel-image-container relative flex justify-center items-center h-screen">
    <div class="absolute">
      <!-- Carousel image -->
      <div>
        <a href={content[contentIdx].link} target="_blank">
          <img id="carousel-img-id" class="carousel-img shadow-xl" width={imgWidth} src={getImgUrl(imgIndex)} alt={content[contentIdx].alt} />
        </a>
      </div>
      <!-- Content -->
    ...
    <div id="carousel-nav-btns" class="absolute left-0 top-1/2 transform -translate-y-1/2">
      <!-- Prev button-->
      <button onclick={btnPrev} class="carousel-nav">
        <svg class="fill-white h-16 w-12" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
          <path d="M0 0h24v24H0z" fill="none"/>
          <path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/>
        </svg>
      </button>
    </div>
    <div id="carousel-nav-btns" onclick={btnNext} class="absolute right-0 top-1/2 transform -translate-y-1/2">
      <!-- Next button-->
    ...
    <div id="carousel-selectors" class="absolute flex bottom-0 left-1/2 transform -translate-x-1/2 mb-4">
      {#each Array(4) as _, idx}
        {#if imgIndex === (idx + 1)}
          <button onclick={() => btnUpdateCarousel(idx)} value={contentIdx} class="px-1.5">
            <svg xmlns="http://www.w3.org/2000/svg" height="14px" width="14px" viewBox="0 0 24 24" class="fill-white">
              <path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2z"/>
            </svg>
          </button>
        {:else}
          <button onclick={() => btnUpdateCarousel(idx)} value={contentIdx} class="px-1.5">
            <svg xmlns="http://www.w3.org/2000/svg" height="14px" width="14px" viewBox="0 0 24 24" class="fill-slate-300">
              <path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2z"/>
            </svg>
          </button>
        {/if}
...
{:else}
  ...
  {#each content as item}
    <div class="flex justify-center items-center h-550" style="background-color: {item.background}">
      <div class="flex flex-col">
        <div>
          <a href={item.link} target="_blank">
            <img class="pt-6 shadow-xl" width={imgWidth} src={getImgUrl(item.id)} alt={item.alt} />
          </a>
        </div>
